...
// 学习 逐顶点光照 Phong模型高光 兰伯特模型漫反射 及 环境光
// 从性能上来说逐顶点优于逐象素 毕竟顶点比像素少的多

Shader "ShaderEnter/001Light_Lambert/Light_Lambert_Vertex_Specular_Diffuse"
{
    Properties
    {
    ...
    SubShader
    {
        Pass
        {
            ...
            v2f vert(a2v v)
            {
                ...
                fixed3 reflectDir = normalize(reflect(-worldLight,worldNormal));//获取世界空间下的反射向量
                fixed3 viewDir = normalize(_WorldSpaceCameraPos.xyz - mul(unity_ObjectToWorld,v.vertex).xyz);//获取世界空间下 顶点朝向摄像机的向量
