...
// 学习 让物体接受阴影 使用Base Pass 和Additional Pass
// 使用逐像素光照 BlinnPhong模型高光 半兰伯特模型漫反射 及 环境光

Shader "ShaderEnter/004RenderPath/ForwardShadow"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag(v2f i) : SV_Target {
				...
				fixed atten = 1.0;//平行光没有衰减 则设为1

				fixed shadow = SHADOW_ATTENUATION(i);//这些内置宏帮助我们在必要时计算光源的阴影
    ...
    FallBack "Specular" //这里调用了VertexLit 中的 pass （LightMode = ShadowCaster）
}
