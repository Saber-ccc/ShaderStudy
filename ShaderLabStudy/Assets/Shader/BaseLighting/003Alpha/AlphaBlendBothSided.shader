...
// 学习 双面透明度混合 深度写入关闭 通过两个pass 以牺牲性能代价正确渲染物体的正面与背面
// 使用逐像素光照 BlinnPhong模型高光 半兰伯特模型漫反射 及 环境光
// 从性能上来说逐顶点优于逐象素 毕竟顶点比像素少的多

Shader "ShaderEnter/002SingleTexture/AlphaBlendBothSided"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            ZWrite Off //深度写入关闭  
            //开启混合一般有两个等式，分别用于计算RGB通道与A通道
            //每个等式有两个因子，分别与源色、目标色相乘
            //源色：该片元着色器产生的颜色  目标色：已经在颜色缓冲中的颜色 因子：SrcAlpha、OneMinusSrcAlpha都是因子
            // 下方只有一个行，因此它实际上RGB和A通道都是用同样的因子 
