...
// 学习 透明度混合 深度写入打开 通过两个pass 以牺牲性能代价解决复杂非凸网格的正确透明渲染
// 使用逐像素光照 BlinnPhong模型高光 半兰伯特模型漫反射 及 环境光
// 从性能上来说逐顶点优于逐象素 毕竟顶点比像素少的多

Shader "ShaderEnter/002SingleTexture/AlphaBlendZWrite"
{
    Properties
    {
    ...
    SubShader
    {
        //RenderType 通常被用于着色器替换功能 这里让Unity把这个Shader归入到提前定义的组（Transparent）
        Tags {"Queue"="Transparent" "IgnoreProjector"="True" "RenderType"="Transparent"}
        Pass
        {
            //这个Pass的目的仅仅是为了把模型的深度信息写入深度缓冲中，从而剔除模型中自身遮挡的片元。
            //因此， Pass 第一行开启了深度写入
            //第二行 ColorMask 用于设置颜色通道的写掩码 当设为0时，不写入任何颜色通道，只需写入深度缓存即可
            ZWrite On
            ColorMask 0
        }
        ...
        Pass
        {
            ...
            ZWrite Off //深度写入关闭  源目标：由该片元着色器产生的颜色 目标颜色：从颜色缓冲中读取的颜色
            Blend SrcAlpha OneMinusSrcAlpha //该片元着色器产生的颜色 * SrcAlpha + 已经在颜色缓冲中的颜色 * (1- SrcAlpha)
