...
namespace Flower{
    ...
    /// <list type="bullet">
    ///     <item>RegisterCommand         : 注册自定义命令.</item>
    ///     <item>RegisterEffect          : 注册自定义效果.</item>
    ///     <item>RegisterToSceneObject   : 将GameObject注册到Flower Scene，您可以将命令应用到此对象.</item>
    ///     <item>GetSceneObject          : 获取花场景对象。.</item>
    ...
    /// <list type="bullet">
    ///     <item>isCompleted             : 输入文本列表是否完全由系统执行.</item>
    ///     <item>text                    : 当前文本结果.</item>
    ///     <item>isWaitingForNext        : 正在等待用户输入->Next（）函数.</item>
    ...
    ///     <item>elementsDestroyOnLoad   : 场景发生变化时，系统和元素会被破坏吗.</item>
    /// </list> 
    /// </summary> 
    public class FlowerSystem : MonoBehaviour
    {
        public bool isCompleted { get { return isTextListCompleted; } }//文本是否输出完成
        public string text { get { return msgText.ToString(); } }
        public bool isWaitingForNext { get { return isWaitingForNextToGo; } }//是否正在等待用户输入Next
        public float textSpeed = 0.01f;//文本输出速度
        public bool isRichTextEnable = true;//是否使用富文本
        public bool isDefaultLogEnable //启用默认日志记录系统
        {
            set {
                if(value){
        ...
        public bool elementsDestroyOnLoad = true;//场景发生变化时，系统和元素会被破坏吗
